...
async function loadImagesConfig() {
    try {
        const response = await fetch('../data/images.json?v=' + Date.now());
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        imagesConfig = await response.json();
        console.log('✅ Images config loaded:', imagesConfig);
        populateImagesForms();
    } catch (error) {
...
async function loadDepartmentsData() {
    ...
    try {
        ...
        const response = await fetch('../data/departments.json?v=' + Date.now());
...
async function loadProgramsData() {
    try {
        const response = await fetch('../data/programs.json?v=' + Date.now());
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
...
function showAddDepartmentForm() {
    ...
    document.getElementById('departmentLeadersDescription').value = '';
    document.getElementById('departmentLeader1Name').value = '';
    document.getElementById('departmentLeader1Role').value = '';
    document.getElementById('departmentLeader1Image').value = '';
    document.getElementById('departmentLeader2Name').value = '';
    document.getElementById('departmentLeader2Role').value = '';
    document.getElementById('departmentLeader2Image').value = '';
    document.getElementById('departmentVision').value = '';
    document.getElementById('departmentEligibility').value = '';
    
    // Clear activities
    document.getElementById('departmentActivitiesContainer').innerHTML = '';
...
function editDepartment(id) {
    ...
    // Leaders
    document.getElementById('departmentLeadersDescription').value = dept.leadersDescription || '';
    if (dept.leaders && dept.leaders.length > 0) {
        document.getElementById('departmentLeader1Name').value = dept.leaders[0].name || '';
        document.getElementById('departmentLeader1Role').value = dept.leaders[0].role || '';
        document.getElementById('departmentLeader1Image').value = dept.leaders[0].image || '';
        if (dept.leaders.length > 1) {
            document.getElementById('departmentLeader2Name').value = dept.leaders[1].name || '';
            document.getElementById('departmentLeader2Role').value = dept.leaders[1].role || '';
            document.getElementById('departmentLeader2Image').value = dept.leaders[1].image || '';
        }
    }
    
    // Vision
    document.getElementById('departmentVision').value = dept.vision || '';
    
    // Eligibility
    if (dept.eligibility && Array.isArray(dept.eligibility)) {
        document.getElementById('departmentEligibility').value = dept.eligibility.join('\n');
    } else {
        document.getElementById('departmentEligibility').value = '';
    }
    
    // Activities
    const activitiesContainer = document.getElementById('departmentActivitiesContainer');
    activitiesContainer.innerHTML = '';
    if (dept.activities && Array.isArray(dept.activities)) {
        dept.activities.forEach((activity, index) => {
            addActivityField(activity.title, activity.description, activity.image);
        });
    }
...
function saveDepartment() {
    ...
    const vision = document.getElementById('departmentVision').value.trim();
    const eligibilityText = document.getElementById('departmentEligibility').value.trim();

    if (!name || !description || !vision || !eligibilityText) {
        alert('Veuillez remplir tous les champs obligatoires (*)');
        return;
    }
    ...
    // Build leaders array
    const leaders = [];
    const leader1Name = document.getElementById('departmentLeader1Name').value.trim();
    const leader1Role = document.getElementById('departmentLeader1Role').value.trim();
    const leader1Image = document.getElementById('departmentLeader1Image').value.trim();
    if (leader1Name) {
        leaders.push({
            name: leader1Name,
            role: leader1Role || '',
    ...
    const leader2Name = document.getElementById('departmentLeader2Name').value.trim();
    const leader2Role = document.getElementById('departmentLeader2Role').value.trim();
    const leader2Image = document.getElementById('departmentLeader2Image').value.trim();
    if (leader2Name) {
        leaders.push({
            name: leader2Name,
            role: leader2Role || '',
    ...
    const eligibility = eligibilityText.split('\n')
        .map(line => line.trim())
        .filter(line => line.length > 0);

    // Build activities array
    const activities = [];
    const activityFields = document.querySelectorAll('.activity-field');
    activityFields.forEach(field => {
        const title = field.querySelector('.activity-title-input')?.value.trim();
        const desc = field.querySelector('.activity-description-input')?.value.trim();
        const img = field.querySelector('.activity-image-input')?.value.trim();
        if (title && desc) {
            activities.push({
                title,
                description: desc,
    ...
    if (image) departmentData.image = image;
    if (leaders.length > 0) departmentData.leaders = leaders;
    const leadersDesc = document.getElementById('departmentLeadersDescription').value.trim();
    if (leadersDesc) departmentData.leadersDescription = leadersDesc;
        ...
        <div class="item-card">
            <div class="item-card-content">
                ...
                <div style="margin-top: 1rem; font-size: 0.9rem; color: #666;">
                    ${dept.vision ? `<div><strong>Vision:</strong> ${dept.vision.substring(0, 100)}${dept.vision.length > 100 ? '...' : ''}</div>` : ''}
                    ${dept.leaders && dept.leaders.length > 0 ? `<div style="margin-top: 0.5rem;"><strong>Responsables:</strong> ${dept.leaders.map(l => l.name).join(', ')}</div>` : ''}
                    ${dept.activities && dept.activities.length > 0 ? `<div style="margin-top: 0.5rem;"><strong>Activités:</strong> ${dept.activities.length} activité(s)</div>` : ''}
                    ${dept.eligibility && dept.eligibility.length > 0 ? `<div style="margin-top: 0.5rem;"><strong>Éligibilité:</strong> ${dept.eligibility.length} critère(s)</div>` : ''}
                </div>
            </div>
            <div class="item-card-actions">
...
// Add activity field dynamically
let activityFieldCounter = 0;
function addActivityField(title = '', description = '', image = '') {
    const container = document.getElementById('departmentActivitiesContainer');
    if (!container) return;
    
    const fieldId = `activity-${activityFieldCounter++}`;
    const activityField = document.createElement('div');
    activityField.className = 'activity-field';
    activityField.style.marginBottom = '1.5rem';
    activityField.style.padding = '1rem';
    activityField.style.border = '1px solid #ddd';
    activityField.style.borderRadius = '5px';
    activityField.style.backgroundColor = '#f9f9f9';
    
    activityField.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">
            <h6 style="margin: 0; color: var(--color-secondary);">Activité</h6>
            <button type="button" class="btn-remove" onclick="removeActivityField(this)" style="padding: 0.25rem 0.5rem; font-size: 0.85rem;">
        ...
        <div class="form-grid">
            <div>
                ...
                <input type="text" class="activity-title-input" placeholder="Ex: Enseignements bibliques" value="${title}" required>
            </div>
            <div>
                <label>URL de l'image</label>
                <input type="text" class="activity-image-input" placeholder="URL de l'image" value="${image}">
            </div>
        </div>
        <div style="margin-top: 1rem;">
            <label>Description *</label>
            <textarea class="activity-description-input" rows="2" placeholder="Description de l'activité..." required>${description}</textarea>
        </div>
    `;
...
function removeActivityField(button) {
    if (confirm('Êtes-vous sûr de vouloir retirer cette activité ?')) {
        button.closest('.activity-field').remove();
    }
}
